...
if ($saveDB) {
    $picture = \xinyeweb\images\models\Picture::find()->where(['id'=>$data])->asArray()->one();
    if (!$picture) {
        $picture['id']   = '';
        $picture['path'] = '';
    }
...
<div class="fileinput fileinput-new" data-provides="fileinput">
    ...
    <div class="fileinput-preview thumbnail" data-trigger="fileinput" style="width: 200px; height: 150px;">
        <img class="file_img" src="<?= !empty($picture['path']) ? \xinyeweb\images\FuncHelper::src($picture['path']) : \yii\helpers\Url::to('@web/assets/static/no.png'); ?>">
    </div>
</div>

<?php
/* 加载页面级别资源 */
\xinyeweb\images\FileinputAsset::register($this);
?>
                ...
                $.ajax({
                    ...
                    success: function(json){
                        if(json.boo){//console.log(file_ipt);console.log(file_img);
                            file_img.attr('src','/uploads/'+json.data.url);
